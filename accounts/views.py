...
from .decorateurs import admin_required
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if password1== password2:
                    if (CommonUser.objects.filter(email=email).exists() or CommonUser.objects.filter(phone=phone).exists()):
                        form.add_error(None, 'Ces identifiants sont déjà utilisés.')
                    else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if status:
                ...
                response.save()
                send_mail(
                    'Votre plainte a bien été traité',
                    f"{msg}",
                    'arsenenikiema685@gmail.com',
...
def log_out(request):
    logout(request)
    response = HttpResponse("Oups ! vous avez été déconnecté.")
    response.delete_cookie('sessionid')
    response.delete_cookie('csrftoken')

    response = redirect('login_email')
    response.delete_cookie('sessionid')
    response.delete_cookie('csrftoken')
    return response
